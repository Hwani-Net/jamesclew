...
> 버전: 2026-04-14 | Claude Code v2.1.107 | 하네스 위치: `D:/jamesclew/harness/`
...
| `/audit` | 세션 하네스 준수 감사 | `audit-session.sh` 실행, 23개 항목 리포트 |
...
| `Stop` | `stop-dispatcher.sh` | `enforce-execution` + `evidence-first` 등 5개 검증 통합 |
...
| 스크립트 | 용도 | 사용법 |
|---------|------|--------|
| `audit-session.sh` | 세션 하네스 준수 23개 항목 감사 | `bash audit-session.sh --full` / `--compact` |
