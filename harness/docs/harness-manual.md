...
3. **Generator ≠ Evaluator**: 자기 검수 편향 차단. 생성한 모델이 직접 검수하지 않고 외부 모델(Codex 1순위, gemma4 보조)이 평가.
...
`deploy.sh`가 배포하는 항목: `CLAUDE.md`, `settings.json`, `rules/`, `hooks/`, `scripts/`, `agents/`, `commands/` (PITFALLS는 `harness/pitfalls/`에 직접 파일로 관리 — `grep -ri "키워드" D:/jamesclew/harness/pitfalls/`로 검색)
...
│                  외부 모델 교차검수                           │
│  ┌───────────────┐  ┌───────────────┐  ┌─────────────────┐  │
│  │  Codex CLI    │  │  gemma4 (보조) │  │  Gemma 4 로컬   │  │
│  │  (6계정 로테) │  │  Ollama:11434  │  │  Ollama:11434   │  │
│  └───────────────┘  └───────────────┘  └─────────────────┘  │
└─────────────────────────────────────────────────────────────┘
                                │
                        ┌───────┘
...
├── pitfalls/              # 반복 실수 기록 (pitfall-NNN-*.md) — grep으로 직접 검색
├── deploy.sh              # ~/.claude/ 배포
...
| `/qa` | 배포 결과물 외부 모델 QA 루프 | Codex(1순위)/gemma4(보조) 사용자 관점 평가 → 수정 → 재평가 반복 |
...
| `Bash` (firebase deploy) | `enforce-review.sh` | 외부 모델(Codex 1순위 + gemma4 보조) 검수 리마인더 주입 |
...
| `PreCompact` | ~~`gbrain sync`~~ | ~~레포 gbrain 동기화~~ (gbrain 폐기됨 — P-172) |
...
**도구 우선순위 (비용 효율순):**
```
외부 모델(Codex 1순위/Gemma4 보조, 5H=0) > 서브에이전트(Sonnet) > Built-in > Bash > MCP
```
...
| gemma4 (Ollama, 보조 전용) | `curl http://localhost:11434/api/chat` | 보조 의견, 폴백 | 콘텐츠 보조 의견, 단독 판단 금지 |
...
**주의:** 로컬 모델(gemma4 등)은 보조 의견 전용 — 단독 PASS/FAIL 판단 금지. Codex CLI가 1순위 외부 검수. Ollama 서버 실행 필수 (`ollama run gemma4 &`).
...
|------|---------|---------|----------|
| Codex / Gemma4 (보조) | **0** | **0** | 코드 리뷰, 외부 검수 — Codex 1순위, gemma4 보조 의견만 |
...
- **위치:** `/tmp/HydraTeams/`
- **포트:** 3456 (Codex CLI / Ollama `4141`과 구분)
- **역할 분리:** Codex CLI / Ollama(`4141`) = 단일 API 검수 호출 / HydraTeams(`3456`) = Agent Teams teammate 전용
...
### 자율 저장 규칙 (옵시디언 + agentmemory)

지식이 발생하는 즉시 저장하는 것이 원칙이다.

**자동 저장 트리거:**

| 조건 | 저장 방법 |
|------|----------|
| 새로운 도구/기법 발견 | `$OBSIDIAN_VAULT/05-wiki/entities/{slug}.md` 직접 작성 |
| 디버깅 핵심 원인 발견 | `harness/pitfalls/pitfall-NNN-{slug}.md` 작성 |
| 외부 API/서비스 연동 확인 | `$OBSIDIAN_VAULT/05-wiki/sources/{slug}.md` 작성 |
| 대표님 "기억해" / "저장해" 요청 | 위치 적절 선택 + `mcp__agentmemory__memory_save` |

**저장 방법:**
```bash
# 옵시디언 직접 작성
Write "$OBSIDIAN_VAULT/05-wiki/entities/{slug}.md"

# agentmemory MCP
mcp__agentmemory__memory_save(content="...", metadata={...})
```

**Search-Before-Solve 순서:** 막히면 `grep -ri "키워드" $OBSIDIAN_VAULT/05-wiki/` 또는 `mcp__agentmemory__memory_recall` 우선 (PITFALLS pitfall-NNN-* 포함) → 이전 세션 순서로 검색 후 해결 시도.
...
- **주 입력 경로:** `/inbox-process` 커맨드로 00-inbox를 분류하거나 직접 마크다운 작성 (`03-knowledge/`에 저장)
- **수동 큐레이션도 허용:** 판단·설계 철학은 직접 마크다운으로 작성
...
### ~~gbrain~~ [DEPRECATED 2026-05-19]

> ⚠️ gbrain은 P-172 결정으로 폐기됨. 대체: **옵시디언 직접 grep + agentmemory MCP**.
> - 검색: `grep -ri "키워드" $OBSIDIAN_VAULT/05-wiki/`
> - 저장: `mcp__agentmemory__memory_save` 또는 옵시디언 직접 파일 작성
...
| 변수 | 용도 |
|------|------|
| `OPENAI_API_KEY` | Codex CLI 호출 (gemma4는 Ollama, 키 불필요) |
...
> **PITFALLS 검색**: `grep -ri "증상키워드" D:/jamesclew/harness/pitfalls/`
> 또는 `mcp__agentmemory__memory_recall`로 세션 기억 검색

| 코드 | 증상 | 파일 |
|------|------|------|
| P-001 | headless 브라우저에서 이미지 미로드 | `pitfall-001-lazy-loading.md` |
| P-002 | 쿠팡 이미지에 UI 오버레이 캡처됨 | `pitfall-002-coupang-image-overlay.md` |
| P-007 | compact 후 세션 저장하면 의미 없음 | `pitfall-007-compact-save-timing.md` |
| P-026 | bypassPermissions으로도 harness hook 우회 불가 | `pitfall-026-bypass-permissions-hook.md` |
| 전체 | — | `grep -ri "pitfall" D:/jamesclew/harness/pitfalls/` |
