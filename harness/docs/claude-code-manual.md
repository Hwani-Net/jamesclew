...
sources:
  ...
  - harness/CLAUDE.md (v2.1.112~v2.1.142 섹션)
...
> 최종 갱신: 2026-05-15 | Claude Code v2.1.142 기준
...
2. [버전 히스토리 (v2.1.85~142)](#2-버전-히스토리)
3. [핵심 개념](#3-핵심-개념)
...
### v2.1.124 ~ v2.1.142 통합 (2026-04-29 ~ 2026-05-15)

> 19개 마이너 통합 — 핵심 변화만. 전체 원문은 `~/.claude/cache/changelog.md`. v2.1.134/v2.1.135는 changelog 자체가 비어있음 (skipped).

#### v2.1.142 (2026-05-15, latest)

| 항목 | 내용 |
|------|------|
| **`claude agents` 신규 플래그 8개** | `--add-dir`, `--settings`, `--mcp-config`, `--plugin-dir`, `--permission-mode`, `--model`, `--effort`, `--dangerously-skip-permissions` — 디스패치된 background sessions 구성 |
| **Fast mode default Opus 4.6 → Opus 4.7** | 이전 Opus 4.6 동작 원하면 `CLAUDE_CODE_OPUS_4_6_FAST_MODE_OVERRIDE=1` |
| Plugin root-level `SKILL.md` 단독 surfacing | `skills/` 하위 디렉토리 없어도 skill로 인식 |
| `/plugin details`, `claude plugin details` LSP 서버 표시 | |
| `/web-setup` 기존 GitHub App connection 교체 전 경고 | |
| **`MCP_TOOL_TIMEOUT` HTTP/SSE 60초 캡 fix** | 이전엔 설정값 무시하고 60초 강제. 이제 정상 적용 |
| Background sessions pre-existing worktree 인식 fix | EnterWorktree가 중복 생성 거부하면서 Edit 차단되던 버그 |
| Background sessions macOS sleep/wake 후 사라지던 fix | daemon이 clock jumps 감지 |
| Daemon binary upgrade 후 클린 exit fix | `brew upgrade` 시 dispatched agents crash-loop 해소 |
| Background agents Claude-in-Chrome 확장 shared tab 없을 때 crash-loop fix | |
| `claude agents` 첨부 세션 링크 클릭 fix | headless browser shim 해제 |
| `claude agents` "v to open in editor" `$EDITOR`/`$VISUAL` 우선 fix | |
| `claude agents` Windows network-drive 디렉토리 deadlock fix | Ctrl+C 시작 시 작동 |
| `claude --bg --dangerously-skip-permissions` retire/wake 영구화 fix | |
| 세션 타이틀 첫 메시지가 링크일 때 URL에서 파생되던 fix | |
| Redundant `set_model` 요청으로 transcript에 중복 `/model` breadcrumbs fix | |
| **Reactive compaction 첫 summarize 시 overflow size seed** | wasted near-full-context retry 회피 |
| Hook 구성 에러 명확화 | `SessionStart`/`Setup`/`SubagentStart`에 prompt/agent type hook 설정 시 "use command-type instead" |
| Stale `/model claude-sonnet-4-20250514` 제안 제거 | Usage Policy refusal 메시지 |

> **하네스 영향**:
> 1. `MCP_TOOL_TIMEOUT` fix는 우리 원격 MCP(tavily 등)에서 60초 초과 작업 가능 — `settings.json`에 명시한 timeout이 비로소 효력 발휘.
> 2. Fast mode default Opus 4.7 전환 — `/fast` 사용 시 자동으로 더 강력. 우리 settings.json에 fast mode pin 없으므로 자연스럽게 4.7.
> 3. `claude agents` 플래그 풍부화 — 향후 background dispatch 자동화 시 활용.
> 4. Reactive compaction 개선 — Opus 1M 세션에서 효율 ↑.

#### v2.1.141 (2026-05-13~14 추정)

| 항목 | 내용 |
|------|------|
| **Hook `terminalSequence` JSON output** | hooks가 desktop notification, window title, bell을 controlling terminal 없이 emit 가능 |
| `CLAUDE_CODE_PLUGIN_PREFER_HTTPS` env | SSH 키 없는 환경에서 GitHub plugin HTTPS clone |
| **`ANTHROPIC_WORKSPACE_ID`** | workload identity federation — federation rule이 multi-workspace 커버할 때 토큰 scope 지정 |
| `claude agents --cwd <path>` | 디렉토리별 세션 리스트 스코프 |
| `/feedback` 최근 세션(24h/7d) 포함 | 다중 세션 걸친 이슈 보고 |
| **Rewind menu "Summarize up to here"** | 이전 컨텍스트 압축 + 최근 turn 유지 |
| Auto mode permission dialog 설명 강화 | `permissions.ask` rule 원인 표시 |
| IDE diff view 복원 | file-edit permission prompts에서 |
| `/bg`/`←←` background agents permission mode 보존 | 이전엔 default로 회귀 |
| `claude agents` Completed 분류 정정 | 작업 끝났지만 background shell 남은 경우 |
| Spinner 10초 후 amber 색상 | "still working" 시그널 |
| 다수 fix | `/model` 동시 세션 autocompact threshold 영향 fix, 권한 모드 전환 시 prompt 자동 dismiss, MCP 403 "needs auth" 표시, **markdown table 셀 wrapping vertical fallback regression(v2.1.136) fix**, voice keybindings unbind fix, Windows Alt+V image paste fix, SDK glibc/musl 충돌 fix 등 |

> **하네스 영향**: `terminalSequence` hook output으로 비-터미널 알림(데스크톱 토스트 등) 직접 가능 — `telegram-notify.sh` 대안 검토 가능. markdown table regression fix는 우리 환경에 잠재 영향 (v2.1.136 이후 출력 깨짐 가능성 해소).

#### v2.1.140 (2026-05-12 추정)

| 항목 | 내용 |
|------|------|
| **Agent tool `subagent_type` case/separator insensitive 매칭** | `"Code Reviewer"` → `code-reviewer` 자동 매칭 — 우리 Agent 호출 편의 ↑ |
| Agent color palette 업데이트 | |
| `/goal` `disableAllHooks`/`allowManagedHooksOnly` 메시지 | hook 비활성 시 클리어 메시지 |
| Settings hot-reload symlink 오인 fix | spurious `ConfigChange` hooks 해소 |
| `claude --bg` "connection dropped" fix | idle-exit 직전 |
| Background service startup enterprise endpoint security 시간 ↑ | |
| Remote managed settings 401 retry fix | force-refreshed token 1회 재시도 |
| Managed `extraKnownMarketplaces` auto-update 영구화 fix | `known_marketplaces.json` |
| **`/loop` redundant wakeups fix** | 이미 notify하는 background task에 polling 안 함 |
| Windows `where.exe` re-spawn stall fix | `gh` 부재 시 |
| `Read` tool `offset` whitespace/`+` prefix string fix | validation 통과 |
| Plugin default folder shadowed 경고 | `plugin.json` 동일 키로 commands/ 등 무시 시 `/doctor`, `claude plugin list`, `/plugin` 표시 |

> **하네스 영향**: `Agent` 호출 시 subagent_type을 우리 agent 파일명(`code-reviewer.md` 등)과 일치 안 시켜도 자동 매칭 — 코드 가독성 ↑. `/loop` redundant wakeup fix는 우리 R14 watchdog과 보완.

#### v2.1.139 (2026-05-11 추정, 대형 신기능)

| 항목 | 내용 |
|------|------|
| **`claude agents` Agent View (Research Preview)** | 모든 Claude Code 세션 한눈에 — running, blocked-on-you, done. docs: code.claude.com/docs/en/agent-view |
| **`/goal` 커맨드** | completion condition 설정 → Claude가 turn 넘어 작업 지속. interactive, `-p`, Remote Control. live elapsed/turns/tokens 오버레이 |
| `/scroll-speed` | mouse wheel scroll speed live preview |
| `claude plugin details <name>` | 컴포넌트 인벤토리 + per-session token cost 추정 |
| Transcript navigation | `?` (shortcuts), `{`/`}` (prompt 점프), `v` (panel toggle) |
| **Hook `args: string[]` field (exec form)** | command를 셸 없이 직접 spawn — path placeholders quoting 불필요. 보안 + 신뢰성 |
| **Hook `continueOnBlock: true` (PostToolUse)** | hook rejection reason을 모델에 피드백하고 턴 계속 진행 — 자동 교정 패턴 |
| **MCP stdio servers `CLAUDE_PROJECT_DIR` env 주입** | 플러그인 configs에서 `${CLAUDE_PROJECT_DIR}` 참조 가능 |
| Compaction prompt sensitive instructions 보존 | |
| `/mcp` Reconnect `.mcp.json` hot-reload | restart 불필요 |
| `/context all` per-skill 토큰 추정 tokenizer 반영 | |
| `claude plugin install <name>@<marketplace>` auto-refresh + retry | |
| `/plugin` installed details hook event/MCP 이름 클린 표시 | |
| `/context` plugin-sourced skills 출처 표시 | |
| Remote MCP transient failure 재연결 retry 전체 사용자 활성 | |
| API requests `x-claude-code-agent-id` / `x-claude-code-parent-agent-id` 헤더 | OTEL spans `agent_id` / `parent_agent_id` |
| Remote Control / `/schedule` / claude.ai MCP connectors 비활성 조건 | `ANTHROPIC_API_KEY`/`apiKeyHelper`/`ANTHROPIC_AUTH_TOKEN` 설정 시 (Claude.ai 로그인 있어도) |
| `forceRemoteSettingsRefresh` 정책 deadlock fix | `claude auth login`/`logout`/`status` 복구 가능 |
| `autoAllowBashIfSandboxed` shell expansion fix | `$VAR`/`$(cmd)` 자동 승인 |

> **하네스 영향**:
> 1. **Hook `args` exec form**은 우리 hook 다수에 적용 가치 — `bash -c "command \"$path\""` → `command [\"$path\"]`로 안전성 ↑. 점진 마이그레이션 후보.
> 2. **Hook `continueOnBlock`**은 `test-manipulation-guard.sh`, `regression-guard.sh` 같은 차단 hook에 적용 시 — 차단 사유를 모델에 피드백해 자동 수정 유도 가능. 강력한 패턴.
> 3. **`CLAUDE_PROJECT_DIR` MCP stdio env**: 우리 MCP(`tavily`, `gbrain` 등) 설정에서 `${CLAUDE_PROJECT_DIR}` 활용 가능.
> 4. **`claude agents` Agent View**: Ralph Loop, 장기 background 작업 가시성 ↑ — 운영 도구로 활용 검토.
> 5. **`/goal` 커맨드**: 자동 진화 루프, Self-Heal 등 다중 턴 자율 작업과 결합 가능.
...
| 항목 | 내용 |
|------|------|
| **VS Code 확장 Windows 활성화 실패 fix** | 번들 SDK `createRequire` polyfill 버그 — Windows에서 확장이 활성화 안 되던 문제 해결 |

#### v2.1.136 (2026-05-08~09 추정, 60+ fixes)

| 항목 | 내용 |
|------|------|
| **`settings.autoMode.hard_deny`** 신규 | auto mode classifier rules가 user intent / allow exception **무조건 무시하고 차단**. soft_deny와 다름 — 절대 차단용. 위험 작업 게이트로 활용 |
| **plan mode + `Edit(...)` allow rule fix** (보안) | matching `Edit(...)` allow rule 존재 시 plan mode가 file writes 차단하지 못하던 우회 봉쇄. P-076 라인 잠재 우회 제거 |
| MCP `.mcp.json`/플러그인/connectors `/clear` 후 사라짐 fix | VS Code 확장, JetBrains 플러그인, Agent SDK에서 silently disappearing |
| MCP OAuth 다중 서버 refresh token race fix | 동시 refresh로 토큰 lost — **일일 재인증 필요 없어짐** |
| extended thinking + redacted block + tool call → 400 fix | API 에러 |
| **`--resume`/`--continue` underscore 경로 fix** | 프로젝트 경로에 underscore 있을 때 세션 못 찾던 버그 |
| `AskUserQuestion` array multi-select 답변 discarded fix | array 형태 답변이 silently 폐기되던 문제 |
| **MCP tool results invisible when server returns content blocks** fix | 서버가 content blocks 반환 시 결과 안 보이던 버그 |
| `CLAUDE_ENV_FILE` SessionStart hooks env vars stale after `/resume`/`/clear` fix | env vars가 세션 재개 시 갱신 안 되던 문제 |
| WSL2 image paste from Windows clipboard PowerShell fallback | xclip/wl-paste 실패 시 |
| 동시 OAuth credential write로 인한 로그인 루프 fix | refresh token race로 fresh token이 덮어쓰여 강제 재로그인되던 버그 |
| 플러그인 `Stop`/`UserPromptSubmit` hooks cache cleanup race fix | 실행 중 세션이 사용 중인 버전을 cache cleanup이 삭제하던 문제 |
| `/usage` weekly reset 캘린더 날짜 표시 | 시간 표시 → 날짜 |
| `/branch` multi-line title fix | pasted multi-line 이름 처리 |
| `CronList` qualifiers + scheduled prompt 표시 fix | 출력 누락 보정 |
| `/clear <name>` cleared session label fix | `/resume` 라벨 누락 |
| `CLAUDE_CODE_ENABLE_FEEDBACK_SURVEY_FOR_OTEL` env | 엔터프라이즈 OTel 캡처 시 세션 품질 설문 재활성 (개인 사용자 무관) |
| 60+ minor UI/렌더링 fixes | CJK 컬러 아티팩트, 슬래시 명령 자동완성, plugin slash command 공백 처리, ReasonML diff 등 |

> **하네스 영향**:
> 1. `autoMode.hard_deny`는 R6 보안 hook과 보완 가능 — 향후 sandbox 강화 시 검토.
> 2. **plan mode + Edit allow rule 보안 fix**는 우리 `enforce-build-transition.sh`와 충돌 없음 (오히려 안전 강화).
> 3. MCP OAuth 다중 서버 refresh fix로 우리 환경(tavily, expect, perplexity 등)에서 일일 재인증 부담 해소.
> 4. `AskUserQuestion` multi-select fix는 우리 사용 패턴에 직접 영향 — 이전엔 multiSelect 결과가 가끔 폐기됐을 가능성.

---

#### v2.1.133 (2026-05-08)

> 공식 changelog 발표됨 (이전 매뉴얼은 미반영 상태로 기록). 핵심 신규:

| 항목 | 내용 |
|------|------|
| `worktree.baseRef` 설정 (`fresh`/`head`) | `--worktree`, `EnterWorktree`, agent isolation worktree의 base 선택. **default `fresh`** — 2.1.128부터의 local HEAD 동작이 origin/<default>로 복귀. unpushed commits 보존하려면 `head` 명시 |
| `sandbox.bwrapPath` / `sandbox.socatPath` | Linux/WSL custom binary 경로 |
| `parentSettingsBehavior` admin-tier 키 | `'first-wins' \| 'merge'` — SDK `managedSettings` 정책 머지 |
| **Hooks `effort.level` JSON input + `$CLAUDE_EFFORT` env** | hook과 Bash subprocess에서 active effort level 참조 가능 |
| 병렬 세션 401 dead-end fix | refresh-token race로 shared credentials 삭제되던 버그 |
| `Edit`/`Write` allow rules drive root (`C:\`) / POSIX `/` 매칭 fix | 항상 prompt하던 버그 |
| `HTTP(S)_PROXY`/`NO_PROXY`/mTLS MCP OAuth 전 단계 적용 fix | discovery, registration, exchange, refresh 전부 |
| Read/Write/Edit mapped network drive `--add-dir`/SDK fix | 거부되던 문제 |
| Remote Control stop/interrupt full cancel fix | 큐 메시지 advance 안 되던 버그 |
| `/effort` 세션 간 영향 fix | 한 세션의 `/effort`가 다른 동시 세션 effort 변경하던 버그 |
| 서브에이전트 project/user/plugin skills 발견 fix | Skill tool에서 스킬 미발견 |

> **하네스 영향**: `worktree.baseRef` default가 `fresh`로 복귀 — Agent isolation worktree에서 unpushed commits 보존하려면 `~/.claude/settings.json`에 `"worktree": {"baseRef": "head"}` 명시 필요. `effort.level` hook 입력으로 `tool-duration-monitor.sh` 같은 hook이 effort 기반 분기 가능.
...
> **하네스 영향**: HydraTeams(`localhost:3456`) 사용 시 `/model` 자동 picker가 opt-in으로 변경됨. 모델 선택 동작이 달라졌다면 `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1` 추가.
...
| `/model` picker + custom base URL | `ANTHROPIC_DEFAULT_*_MODEL_NAME/_DESCRIPTION` override 지원 (HydraTeamsilot-api/HydraTeams 게이트웨이 모델 라벨 커스터마이징) |
...
### 7.3 ANTHROPIC_BASE_URL 프록시 — DEPRECATED 2026-05

> ⚠️ copilot-api는 GitHub 차단으로 2026-05 사용 불가. 대체: HydraTeams(`localhost:3456`).
...
| 프록시 | 포트 | 용도 |
|--------|------|------|
| Ollama | 11434 | 단일 API 호출 (검수, AI냄새) |
