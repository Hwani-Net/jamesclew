...
**외부 모델(Codex CLI, 로컬 Ollama)은 5H와 7D 양쪽 모두 0 소비합니다.** 이것이 라우팅 전략의 핵심입니다. 검수·리뷰·반복 작업을 외부 모델로 위임할수록 Opus 7D 풀이 보존되어 설계·판단에 집중할 수 있습니다. **단, 로컬 모델(gemma4, exaone3.5, glm 등)은 보조 의견만 — 단독 검수/판단 금지 (할루시네이션 위험).**
...
| gemma4 (Ollama, 보조 전용) | `curl -s --max-time 30 http://localhost:11434/api/chat` | 보조 의견, 단독 판단 금지 | 5H 0, 7D 0 |
| Gemma 4 로컬 | Ollama API `localhost:11434` | 무제한, 오프라인 | 0 |
| GLM-5.1 클라우드 | `ollama run glm-5.1:cloud` | 무료, 고성능 | 수동 호출만 (cloud 과금 리스크) |

gemma4 보조 의견 호출 전 Ollama 서버가 실행 중이어야 합니다: `ollama run gemma4` (1순위는 Codex CLI)
...
| 코드 리뷰 | Codex (1순위) + gemma4 (보조 의견) | 의견 불일치 시 Opus 최종 판단 |
...
**HydraTeams 프록시**: Agent Teams의 teammate를 외부 모델로 라우팅합니다.
...
## 7. GPT 메인 전환 — DEPRECATED 2026-05

> ⚠️ copilot-api는 GitHub 외부 접근 차단으로 2026-05부터 사용 불가. 이 섹션은 역사 참조용으로만 보존.
> 대체: Ollama 로컬 모델(gemma4)을 보조 의견으로만 사용. 단독 판단 금지.

```bash
# 보조 의견용 Ollama (1순위는 Codex CLI)
ollama run gemma4 &
```
...
- P-029: Ralph Loop + Codex CLI / Ollama 연동 패턴
