...
| **SessionStart** | 세션 시작 | telegram-notify start + 규칙 주입, Codex CLI / Ollama-autostart, session-start (TS) |
...
### C. GPT 메인 전환 지원 (Codex CLI / Ollama-autostart.sh)
SessionStart에서 Codex CLI / Ollama 서버(localhost:11434) 가동 상태를 자동 확인합니다. 5H 한계 도달 시 `ANTHROPIC_BASE_URL=http://localhost:11434 claude`로 새 세션을 열면 GPT-4.1이 Claude Code의 모든 도구를 사용하는 메인 모델로 전환됩니다. 서버 기동을 hook이 보장하므로 전환 직전 수동 시작이 불필요합니다.
