---
description: "외부 모델 QA 루프"
---

# /qa — 외부 모델 QA 루프

배포된 결과물을 **외부 모델(Codex 1순위, gemma4 보조)이 사용자 관점**으로 평가하고,
...
**1-2. 로컬 자동 스캔**
```
1. gstack /browse 스크린샷 (데스크톱+모바일): B="$HOME/.claude/skills/gstack/browse/dist/browse.exe" → $B goto <url> → $B screenshot <desktop.png> → $B js "window.resizeTo(390,844)" → $B screenshot <mobile.png>
...
**로컬 루프 규칙:**
- 매 라운드 **다른 외부 모델** 로테이션 (Codex → gemma4 보조 → Codex 재실행)
...
# 자동으로: gstack /browse/node 스크린샷 캡처 → Codex 등급 평가 → PASS/REWORK/FAIL 판정
...
2. gstack /browse 라이브 스크린샷 (데스크톱+모바일): B="$HOME/.claude/skills/gstack/browse/dist/browse.exe" → $B goto <url> → $B screenshot <desktop.png> → $B js "window.resizeTo(390,844)" → $B screenshot <mobile.png>
