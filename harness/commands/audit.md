...
- `/audit --deep` — 통합 + 외부 모델(Codex) 종합 검수
...
세션 + Hook 인프라 결과를 Codex에 위임 (gemma4 보조 의견):
...
# gemma4 보조 의견 (Ollama, 단독 판단 아님)
curl -s --max-time 30 localhost:11434/api/chat \
  -H "Content-Type: application/json" \
  -d "{\"model\":\"gemma4\",\"stream\":false,\"messages\":[{\"role\":\"user\",\"content\":\"같은 프롬프트\"}]}"
```
...
External Review (deep): Codex 권장 사항 + gemma4 보조 의견
