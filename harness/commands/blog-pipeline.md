...
### Step 0: 체크포인트 init/resume (durable-execution, 2026-06-22 — rules/durable-execution.md)
[durable] 키워드별 1 태스크. 크래시·중단 시 **마지막 PASS 스텝부터 재개**(완료 초안 재생성 안 함 = 토큰 절감). 헬퍼 `harness/scripts/task-checkpoint.sh`, 스텝 인덱스 **generate=0·review=1·fix=2·publish=3**.
- 시작: `bash harness/scripts/task-checkpoint.sh resume-point blog-<slug>` → `NO_TASK`면 `… init blog-<slug> "<키워드>" "generate,review,fix,publish"`(cursor=0); 아니면 반환 스텝(예 `2:fix`)부터 시작, 그 미만 스텝은 **skip**(재실행 금지 = Reins PASS 불변).
- 각 스텝 게이트 PASS 직후: `… step-done blog-<slug> <N> <name> PASS "draft=<경로> status=<draft|ready>"`. FAIL이면 step-done FAIL → 재개 시 그 스텝부터.
- 다중 키워드(Step 5): 키워드마다 별도 task-id → 배치 중단돼도 키워드별 재개.
...
⏱️ 총 소요: {duration}
📁 D:/AI 비즈니스/smartreview/drafts/{slug}/
