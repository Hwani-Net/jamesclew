...
| 6 | 이미지+링크 | og:image CDN → gstack /browse(screenshot) → fallback + 대표이미지 적합성 Vision 검증 + 전 외부링크 유효성 확인 |
| 7 | 교차검수 | 외부 모델(Codex 1순위 + gemma4 보조) avg 7/10+ |
| 8 | DB저장 | Firestore에 publish 상태 저장 |
| 9 | 빌드 | CSS + SSG 정적 생성 |
| 10 | 배포 | Firebase Hosting deploy |
| 11 | 검증 | gstack /browse 스크린샷(screenshot) + 디자인 5패스 × 2라운드 saturation. FAIL → 수정 → 재배포 루프 |
...
| 6 | 스크린샷 | gstack /browse(screenshot) 주요 화면 캡처 |
...
| 11 | 검증 | gstack /browse 스크린샷(screenshot) + 디자인 5패스(레이아웃/타이포/시각/인터랙션/렌더링) × 2라운드 saturation. FAIL 시 수정 → 재배포 → 재검증 루프 |
...
- **Step 7 완료 후**: 외부 모델 응답을 `~/.harness-state/step7_review_done`에 기록. **실제 외부 모델(codex exec) 출력을 포함해야 하며, 100byte 미만이면 deploy hook이 차단합니다.**
  ```bash
  # 올바른 예시:
