---
description: "품질 게이트 실패 시 자동 수정 + 에스컬레이션 루프"
---
...
```
gemma4 실패 시 (Ollama 서버 다운 등) Codex CLI 폴백:
```bash
...
- Codex 3회 재시도 실패 + Gemma 4 로컬도 실패 → 즉시 에스컬레이션 (로컬 단독 결정 금지)
