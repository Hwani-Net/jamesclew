...
- **P-266 #작업-요청 과응답 차단 — worker만 requireMention 비대칭** (2026-06-12): #작업-요청(`1508275532851183727`) 단일 질문에 worker 6봇 전원이 각자 직접 응답하는 과응답(09:07 gateway 로그 실측, EVE·Data 139초 stalled). **진짜 원인(소스 결정론 확인)**: 봇별 비대칭은 이미 존재(main=guild requireMention:False, worker=True)했으나 **worker 6봇이 #작업-요청 채널 override를 `requireMention:False`로 명시해 guild의 True를 덮어씀**. OpenClaw `get-reply-*.js`: `resolveDiscordRequireMentionFallback`이 **channelEntry > guildEntry 우선**, `defaultGroupActivation(rm)=!rm?"always":"mention"` → channelEntry=False면 `"always"`(무멘션도 응답)=과응답. **해결**: **worker 6봇만** `accounts.<bot>.guilds.1506254036310167635.channels.1508275532851183727.requireMention: False→True` (→`"mention"`=자기 멘션 시만), **main(default)은 False 유지**(→`"always"`=대표님 무멘션 질문을 JARVIS만 수신→분배). ⚠️**오처방 주의**: top-level `discord.guilds`는 봇별 설정 부재 시 폴백일 뿐 무효 — 진짜 제어점은 `accounts.<bot>.guilds.<gid>.channels`; "전 봇 true"는 JARVIS까지 막아 부작용. **다음 세션: worker의 이 채널 requireMention=True 해제 금지(과응답 재발) + main=False 유지 금지변경(해제 시 JARVIS 분배 불능).** 봇 토큰 REST 검증은 author=bot 한계(allowBots:mentions)로 user 메시지 재현 불가 → 소스 결정론 검증으로 입증. 백업 `openclaw.json.bak-reqmention-CORRECT-*`. 상세: pitfall-266-jakeop-channel-over-response-requireMention.
- **P-266 확장 — 전 14개 공용채널 일괄 (2026-06-14)**: #작업-진행중에서 같은 과응답 재발(worker 6봇 전원 requireMention=False, 18:50 promptChars=713에 sonnet/opus/gemini 다발 실측) → 점검 결과 14개 공용채널 중 12개가 worker False였음. **근본 차단**: 전 14개 공용채널(작업-요청·진행중·완료 / 리뷰-요청·완료 / 결재-필요 / 자료실 / 일반 / 공지사항 / 일반-채팅 / 코드-작업 / 자동화 / 리서치 / 에이전트-설정) **worker 6봇 requireMention=True 일괄(72건)**, **main(default)은 전부 False(always) 유지**. #에이전트-설정도 worker True(컨디션은 JARVIS 종합, 대표님 선택). ⚠️**원칙: 공용채널 worker requireMention override는 True(또는 미생성=guild True 상속), False override 금지 / main만 always / 신규 채널도 동일.** sessions_send 위임은 requireMention 무관이라 worker True여도 위임 흐름 안 깨짐(실증). 백업 `openclaw.json.bak-reqmention-allpublic-*` / `.bak-reqmention-progress-*`.
...
- **P-267 OpenClaw 인프라 3건** (2026-06-12): ①**Data(ollama) endpoint** — WSL2 **mirrored 모드** 전환(default gw 192.168.50.1)으로 `models.providers.ollama.baseUrl`을 `172.23.192.1:11436`(구 NAT, ETIMEDOUT)→`127.0.0.1:11436` 수정. **다음 세션 Data 다운 시 `ip route show default`로 모드 판별**(192.168.x=mirrored→127.0.0.1 / 172.23.x=NAT→그 IP). ②**`workboard-notify-poll` cron**(id 4c78824a, 10분, main/haiku, `--no-deliver`) 신설 — 하트비트 격리세션의 workboard_notify 미노출 우회(**heartbeat 스키마에 toolsAllow 필드 없음 → 주입 시 gateway 죽음 'agents.list.0.heartbeat: Invalid input'**, cron `--tools`로만 가능). ③`tools.agentToAgent.allow`에 `ollama` 추가. 상세: pitfall-267.
- **`openclaw-stuck-watchdog.timer`** (WSL2 systemd user, 10분 주기, 2026-06-13, P-268) — **JARVIS 세션 idle stuck 자동재개 안전망** (P-201 잔여갭 ④/⑤). 배경: `sessions_send` 위임→worker 반환 후 JARVIS(orchestrator)가 자동 재개 안 됨(idle close stuck, 1사이클 2회 실측). `stuck-watchdog.py`가 **running/claimed 카드의 `MAX(card_events.at)` 20분 초과 = stuck 감지 → `openclaw agent --agent main` 트리거**(60분 쿨다운 debounce, worker runtime 무관 = EVE/Data 커버). ⚠️**옵션B(worker가 message 툴로 @JARVIS 발송) 폐기 이유**: EVE(gemini runtime)·Data(ollama)는 **message 툴 미제공**(EVE 자기보고+검증 실증)이라 B 불가. ⚠️**`openclaw system event --mode now`는 JARVIS 즉시 안 깨움**(90초 0 실측) → 트리거는 반드시 `openclaw agent --agent main`. service `TimeoutStartSec=600`(agent 턴 최대 5.5분 블로킹 대비). 상세: pitfall-268.
...
- **Claude Code 기능 참조 (우선순위 1→3, 2026-04-21 로컬 신뢰 소스 도입)**: 새 기능/도구 도입 전 **및 프로젝트 시작 시** 반드시 조회:
  1. **로컬 매뉴얼 (1차 소스)**: `~/.claude/docs/claude-code-manual.md` (v2.1.177 반영, 메인 세션 Opus 4.8, git 관리). 옵시디언 미러 `$OBSIDIAN_VAULT/01-jamesclaw/harness/docs/claude-code-manual.md`
...
- **0단계 (프로젝트 시작/전환 시 필수 사전 조회, 2026-04-21 신설)**:
  1. 로컬 Claude Code 매뉴얼 Glance: `~/.claude/docs/claude-code-manual.md` (v2.1.177 기반). 최신 기능·제약·버전별 변경 확인
...
| 벌크/반복 작업 | Gemma 4 로컬 | — |
| **Vision 분석 (스크린샷/이미지)** | **메인 세션 최상위 모델(현 Opus 4.8) 직접 Read** 1차 | **`codex-vision` (`codex exec -i image`) 2차 cross-family** — `/codex-vision` skill |

### Vision 라우팅 규칙 (중요)
Sonnet 서브에이전트 실행 중 이미지 분석이 필요하면 **반드시 메인 세션 최상위 모델(현 Opus 4.8)로 라우팅**. Sonnet Vision은 디테일 누락률이 20~30%로 상위 모델 대비 현저히 낮음 (P-055/P-181).
...
Sonnet teammate가 스크린샷을 /tmp/screenshot.png에 저장 → 메인 세션(현 Opus 4.8)에 SendMessage("Vision 분석 요청: path=/tmp/screenshot.png") → 메인 모델이 Read로 직접 이미지 분석 → 결과를 Sonnet에 반환.

또는 메인 세션에서 `Read(image_path)`로 직접 처리.

### Computer Use / Browser 자동화 Vision 이중 패스 (인식률 ↑)
`claude-in-chrome`·`desktop-control`·`gstack /browse`의 스크린샷 기반 클릭은 좌표 추정 오류 빈발. 2단계 전략:
...
- **모델 선택**: 리드=메인 모델(현 Opus 4.8), 구현 teammate=Sonnet(`model: sonnet`), 리뷰 teammate=GPT via HydraTeams(`localhost:3456`)
...
> **⚠️ 표면(surface)별로 다름 (pitfall-269 codex 패턴 — 표면마다 붙는 도구가 다름):**
> - **Claude Desktop / Cowork**: **내장 Claude in Chrome** (paired 확장, 네이티브·스마트)이 기본이자 최선 — 그대로 사용. 내장 도구 `mcp__Claude_in_Chrome__*` / `mcp__Claude_Preview__*`. **브라우저 MCP를 Desktop `claude_desktop_config.json`에 등록하지 말 것** — 경쟁 도구 0개 유지 = "내장 고정"(실측: Desktop mcpServers 브라우저 MCP 0개 = 이미 고정).
> - **CLI / 헤드리스 / OpenClaw**:
>   1. **gstack `/browse` (1순위, 네비게이션+렌더검증 겸용)** — `B="$HOME/.claude/skills/gstack/browse/dist/browse.exe"; $B goto/snapshot -i/click @e30/text/screenshot/js`. 반복은 `/scrape`+`/skillify`. **렌더 검증**도 gstack js로: screenshot(Vision)·`performance.getEntriesByType('resource')`(실패리소스)·`document.images naturalWidth>0`(깨진이미지) = blog-review/pipeline/qa Reins 게이트(P-256).
>   2. **claude-in-chrome (2순위, 승인 필요)** — 실제 로그인 탭 조작 시만.
>   3. Playwright CLI 직접 호출 금지.
> - ⚠️ **expect MCP는 2026-07-01 완전 제거**(대표님 승인 B). 렌더 검증은 gstack js로 이관. **대체제 없어 폐지된 게이트: accessibility_audit·performance_metrics(LCP/CLS)·console error 레벨** — 재도입 필요 시 대표님 확인. 다음 세션 expect 재등록 금지.
...
4. **메인 모델 어드바이저 상시**: 외부 모델/Sonnet이 실행해도, 최종 판단·방향 결정·품질 승인은 메인 모델(현 Opus 4.8)이 수행.
...
7D 주간 풀은 메인/Sonnet **별도** — Agent(model: sonnet)은 메인 모델 7D 풀 보존에 유효. (아래 풀 수치는 Opus 시대 실측 기준 — Opus 4.8 풀 정책은 실측 후 갱신.)
...
2026-05-27 신설. **다음 작업은 메인 모델(현 Opus 4.8) 직접 수행. 서브에이전트 위임 절대 금지:**
...
5. Sonnet 메인 전환 후에도 상위 모델 풀은 보존됨 — 리밋 해제 후 원래 메인 모델(`/model claude-opus-4-8`, STICKY 참조)로 복귀

## Context & Session
- **메인 세션(현 Opus 4.8)**: compact **45%에 옵시디언 세션 저장 → `/compact`**. 저장 없이 compact 금지 (P-007). v2.1.105+: PreCompact hook이 옵시디언 저장 실패 시 `exit 2`로 compact 자동 차단.
- **Sonnet 세션**: compact 제한 없음 (auto). 코딩/배포/버그 수정 등 범위 명확한 작업 전용.
- 컨텍스트 수치는 `telegram-notify.sh heartbeat`로 확인. 추측 금지.

## Model Selection

> **현 메인 세션 모델 = `claude-opus-4-8`** (2026-06-11 STICKY 모델 티어 표 참조). 아래 opusplan/opus/sonnet 항목은 대안 모드·폴백 참고용.
...
버전별 신규 기능·하네스 영향은 1차 소스 매뉴얼을 참조: `~/.claude/docs/claude-code-manual.md` (v2.1.177 반영, 메인 세션 Opus 4.8, git 관리). 옵시디언 미러: `$OBSIDIAN_VAULT/01-jamesclaw/harness/docs/claude-code-manual.md`. CLAUDE.md 본문 changelog는 토큰 절감을 위해 2026-05-27 일괄 슬림다운됨. **v2.1.154 신규**: Opus 4.8(default high effort, 최난도 `/effort xhigh`) + Dynamic Workflows(`/workflows` — 수십~수백 에이전트 백그라운드 오케스트레이션, opt-in 시). **v2.1.159**: 트리거어 `workflow`→`ultracode`. **v2.1.163**: Stop/SubagentStop hook `hookSpecificOutput.additionalContext`(턴 유지 결정론 피드백) + hook `if:"Bash(...)"`가 `$()`/`$VAR` 포함 모든 Bash에 오발동하던 것 수정(우리 verify-deploy/quality-gate/enforce-review 노이즈 해소). **v2.1.166**: `fallbackModel`(overload 시 순차 폴백 최대3) + cross-session `SendMessage` user 권한 분리(OpenClaw 멀티봇 보안) + `MAX_THINKING_TOKENS=0`. 로컬 CLI 2.1.161 → 2.1.168(deny 규칙 `rm -rf ~`/`C:\*` 정상화). **v2.1.169**: `--safe-mode`(hook 격리 진단)·`/cd`(prompt cache 보존 디렉토리 변경)·`disableBundledSkills`·"CLAUDE.md too long" 경고 컨텍스트 비례·Windows(`claude -p` skill scan hang, GCM 팝업)/WSL(agent view) 다수 수정. **v2.1.170**: **Claude Fable 5 출시**(Mythos-class 신규 모델 `claude-fable-5` GA — ⛔ 우리 사용 금지, 폐기 표 참조). **v2.1.172**: 서브에이전트 중첩 스폰(최대 5단계) + 1M 컨텍스트 무크레딧 자동 compact 복구 + `opusplan[1m]` 정상화 + WebFetch 와일드카드 도메인 규칙 수정. **v2.1.173**: Fable 5 `[1m]` suffix 자동 strip(1M 기본, `claude-fable-5`=`claude-fable-5[1m]`) + Windows sandbox 거짓 경고 수정. **v2.1.174**: Workflow `agent()` attribution 수정 + Fable 5 credit 배너 정정 + background env var 상속 수정. **v2.1.176**: 세션 제목 대화 언어로 생성(우리 한국어 지침 정합) + hook `if` Read/Edit/Write 경로 매칭 수정(v2.1.163 후속) + auto mode Opus 4.8 불가 시 Fable 5 폴백. **v2.1.175**: `enforceAvailableModels`(미사용). CLI 2.1.173→2.1.177(2.1.177 changelog 미게재). **2026-06-11 메인 세션 Opus 4.8 채택** (STICKY 모델 티어 표 참조).
