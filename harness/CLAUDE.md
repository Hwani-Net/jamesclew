...
## 브라우저 자동화 도구 우선순위
> [2026-06-22 채택, agent-os landscape #2] **gstack `/browse` 1순위** — 실측 검증(browse.exe 112MB 컴파일 바이너리, ~70+ 명령, ~100-200ms/호출, **컨텍스트 토큰 0**, ref 기반 `@e30` 선택). expect/claude-in-chrome 대비 빠르고 승인부담 없음 → P-169/233/242 브라우저 통증 해소.
1. **gstack /browse (1순위)** — 컴파일 CLI 직접 호출: `B="$HOME/.claude/skills/gstack/browse/dist/browse.exe"; $B goto <url>; $B snapshot -i; $B click @e30; $B text; $B screenshot <path>; $B js <expr>`. 반복 흐름은 `/scrape <intent>`+`/skillify`로 Playwright 스크립트 코드화(재실행 ~200ms). 헤디드 디버그 `$B connect`. ⚠️ 첫 실사용 시 daemon 기동·경로 검증(GSTACK_ROOT는 ~/.claude/skills/gstack).
2. **expect MCP (2순위)** — `mcp__expect__*` (open, screenshot, console_logs, network_requests, playwright, accessibility_audit). allowlist 등록, 승인 불필요. /browse 불가하거나 MCP 워크플로 필요 시.
3. **claude-in-chrome (3순위, 승인 필요)** — 실제 로그인 크롬 탭 조작이 꼭 필요한 경우만. 매 호출 승인.
4. **Playwright CLI 직접 호출 금지** — `/browse` 또는 `mcp__expect__playwright`로 대체.
