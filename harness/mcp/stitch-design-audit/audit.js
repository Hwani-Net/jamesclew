...
// Ported from stitch-design-audit.ts to plain CommonJS JavaScript

// ── Tailwind v4 standard mappings ─────────────────────────────────────────────
const TW_BORDER_RADIUS = {
  none: "0px",
  sm: "2px",
  DEFAULT: "4px",
  md: "6px",
...
const VISUAL_EFFECTS = [
  ...
  {
    css: "linear-gradient",
    tailwind: ["bg-gradient", "from-", "to-", "via-", "linear-gradient"],
  },
 ...
 * @param {string} stitchHtml - Raw HTML from Stitch get_screen
 * @param {string} implCode   - Implementation code (TSX, CSS, or combined)
 * @returns {{ verdict: string, score: number, violations: Array, summary: string }}
 */
function auditDesignCompliance(stitchHtml, implCode) {
  const violations = [];
...
function checkBorderRadiusApproximation(stitchHtml, implCode, violations) {
  ...
  for (const value of refRadii) {
    ...
    const standardRoundedRegex =
      /rounded(?:-(sm|md|lg|xl|2xl|3xl|full))?(?![[\d-])\b/g;
    let implMatch;
...
function checkColorApproximation(stitchHtml, implCode, violations) {
  ...
  for (const hex of stitchColors) {
    ...
    for (const [twHex, twName] of HEX_TO_TW_COLOR.entries()) {
      const dist = colorDistance(hex, twHex);
      if (dist < 30 && dist > 0) {
        const classPattern = new RegExp(
          `(?:text|bg|border|ring|accent|fill|stroke)-${twName.replace("-", "-")}\\b`,
        );
...
function checkDomPreservation(stitchHtml, implCode, violations) {
...
function measureMaxNestingDepth(code) {
  const openTags =
    /<(div|section|main|article|aside|header|footer|nav)\b[^>]*>/gi;
...
function generateAuditSummary(verdict, score, violations) {
  const lines = [];
...
module.exports = { auditDesignCompliance };
