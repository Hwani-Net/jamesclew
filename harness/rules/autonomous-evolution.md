...
1. `~/.harness-state/pitfall_recent.log` 에서 7일 이내 동일 키워드 확인
   - 존재 시: agentmemory memory_save 즉시 실행 후 종료
   - 신규: 2~5단계 진행
2. `grep -ri "<증상 키워드>" D:/jamesclew/harness/pitfalls/` 실행
3. 유사 항목 있으면: 기존 슬러그 보고 후 종료
4. 신규면: `harness/pitfalls/pitfall-NNN-{slug}.md` 생성
5. (선택) `mcp__agentmemory__memory_save`로 회상 인덱싱
6. `pitfall_recent.log` 에 타임스탬프(ISO-8601 UTC)+키워드 append
7. stdout 알림. TELEGRAM_BOT_TOKEN 설정 시 텔레그램 전송

### 제약
- TEST_HARNESS=1 환경에서는 실제 agentmemory/텔레그램 호출 없이 stdout만 출력
...
4. `curl -s --max-time 30 localhost:11434/api/chat` (gemma4, 보조 의견)
   - 실패 시: stderr에 "[self-evolve] Ollama 미응답" 출력
5. 응답 stdout 출력
...
### 제약
- 알림 인프라 장애(Ollama/텔레그램 오프라인)가 hook 실패로 전파되면 안 됨 — exit 0 필수
