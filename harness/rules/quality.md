...
## Reins Engineering — 결정론적 검증 위계 (P-256)

출처: parkjunwoo 12강 (Reins Engineering, https://www.parkjunwoo.com/ko/lecture/). 우리 P-194(거짓'발행가능')·P-255(autoFix 미수렴)·skip_review의 근본 처방. "코딩 에이전트가 작동하는 진짜 이유는 모델 지능이 아니라 루프 안의 결정론적 게이트."

### 검증 위계 (순서 강제)
1. **기계 결정론 검증 (최우선)** — 같은 입력=같은 결과. test / lint / HTTP 200 / `naturalWidth>0` / 스키마 / gate script / grep. 기계가 판정 가능하면 LLM에 묻지 마라.
2. **이종 family 외부 LLM** — 기계가 판정 불가한 영역(톤·AI냄새·전략·카피)만. Codex 1순위(P-163).
3. **동일 family 자기검수 금지** — LLM-as-Judge 거짓 pass 36%(실측), 아첨 재발동 + 동일 사각지대 공유. generator≠evaluator (design_rubric).

판단 기준 한 줄: **"이 출력이 맞는지 기계가 판정 가능한가? Yes→검증기로, No→프롬프트로."**

### 결정론적 피드백 계약 (수렴의 핵심)
검수자(critic)·autoFix가 생성자에게 주는 피드백은 반드시 **국소화된 사실**이어야 한다:
- ✅ `line 41: field 'userId' should be 'user_id'` / `이미지 3/4 naturalWidth=0 (img[2], img[4])` / `FAQ 0개 (최소 2개 필요)`
- ❌ "틀렸다 / 개선 필요 / 수정 불충분 / AI냄새 남음" — **모호한 의견은 과잉교정으로 악화**. 실측: "확실해?"에 정확도 27%p 하락·Claude 98% 번복. "의견 주면 아첨, 사실 주면 수정."
- 피드백 등급: 약함("실패")=방향 없음 / 중간("점수 65")=대략 / **강함("line 41,44 미커버 / img[2],[4] 깨짐")=100% 수렴**.

### 완료 판정 기계화 (래칫)
- 게이트 대상 산출물의 **'완료/발행가능' 선언권을 AI에서 박탈** — 결정론적 기계 게이트만 PASS 판정. "LLM이 말하면 40/527, 기계가 말하면 527/527."
- **PASS는 불변** — 통과 후 되돌리지 않음(진동 방지). 새 변경은 기존 게이트 전부 green 유지가 조건.
- **계약 > 절차** — "이 게이트가 PASS여야 한다"(계약)로 지시, "리뷰해라/TDD해라"(절차) 지양. TDAD 실측: 절차 지시 회귀 9.94%↑ vs 계약 지시 1.82%(70%↓).

## Simplicity First (Karpathy G2 — 과잉공학 방지)
[2026-06-21 채택, karpathy-guidelines G2 distill + Codex·서브에이전트 교차검수] 요청 이상의 코드 금지. 단일 사용처에 추상화·유연성·설정성 추가 금지. 불가능한 시나리오용 에러 핸들링 금지. "200줄인데 50줄로 되면 다시 써라." 자가 점검: **"시니어 엔지니어가 과하다고 할까?" → Yes면 단순화.**
- **단, 자율 게이트·기계 PASS 체계가 항상 우선** (G2는 보조 원칙). "12→45"는 검증 후 확장이지 처음부터 과설계가 아님.
- **G1(불확실 시 질문) 각색**: 엔지니어링 불확실성은 사용자에게 묻지 말고 외부모델(Codex/Sonnet) 합의로 자율 해소 (P-168). 사용자 호출은 결재 5건 + 취향·전략 선택(P-213/P-217)만. "가정은 명시하되, 질문은 외부모델에."
...
1. 캡처 우선순위: og:image CDN 800x800 (1순위) → gstack /browse screenshot(`$B screenshot`) (2순위) → claude-in-chrome (3순위, 승인)
...
5. 메인 모델(현 Opus 4.8)+Sonnet 서브에이전트 교차 검증으로 이미지-제품 매칭 확인
...
10. **링크 유효성 주기적 검증** — 배포 시 전체 외부 링크(쿠팡, 다나와 등) HTTP 응답 확인. 404/301/만료 감지 시 `harness/pitfalls/`에 pitfall 기록 + 자동 수정.
...
| 3 | 독자 관점 | AI냄새 제거 (**한국어 텍스트는 `humanizer` 스킬로 40패턴 S1/S2/S3 검사**), 자연스러운 도입, 공감 포인트 | "AI가 쓴 것 같다" 느낌 0 |
...
| 5 | 사용자 페인포인트 | "이 화면에서 사용자가 막히는 곳은?" 관점, 불편사항 `harness/pitfalls/`에 pitfall 기록 |
...
| 5 | 렌더링 검증 | gstack /browse 풀페이지 스크린샷(`$B screenshot`), 모바일/데스크톱 |
...
- 회귀 감지 시 `harness/pitfalls/`에 pitfall 기록 (아래 절차 참조)

## PITFALLS Auto-Record (피드백 → 실수 기록 자동화) [hook 강제: user-prompt.ts]
대표님 지적 → 에이전트 동의 시 `harness/pitfalls/pitfall-NNN-{slug}.md`로 즉시 기록.
...
- **기록 절차**:
  ...
  3. (선택) `mcp__agentmemory__memory_save`로 회상 인덱싱
